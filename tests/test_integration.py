...
from constrai import (
    ...
    InverseAlgebra, GradientTracker, ActiveHJBBarrier, CaptureBasin,
)
...
def test_inverse_algebra_t7():
    ...
    print(f"✅ T7 VERIFIED: undo(execute(s,a)) == s")
    
    # Test rollback record
    record = InverseAlgebra.make_rollback_record(action, s0, s1, 123.456)
    restored = record.apply_rollback(s1)
    assert restored == s0, f"Rollback record failed"
    print(f"✅ Rollback record: snapshot + inverse effects → exact recovery")
...
def test_gradient_tracker_safety_margins():
    ...
    print(f"✅ Safe state detected correctly")
    ...
    print(f"✅ Critical detection working")
...
def test_active_hjb_barrier_reachability():
    ...
    print(f"✅ Dangerous action rejected by HJB barrier")
...
def test_orchestrator_full_integration():
    ...
    print(f"  ✓ Gradient Tracker: {orch.gradient_tracker}")
    print(f"  ✓ HJB Barrier: {orch.hjb_barrier}")
    ...
    for inv in task.invariants:
        ...
        print(f"  ✓ Invariant {inv.name} holds")
    
    # Verify budget safety
    assert result.total_cost <= task.budget, "Budget exceeded!"
    print(f"  ✓ Budget safe: ${result.total_cost:.2f} ≤ ${task.budget:.2f}")
    
    print(f"\n✅ FULL INTEGRATION TEST PASSED")
    print(f"   Theory-to-Execution gap is CLOSED")
