"""
constrai.adapters.langchain_tool — LangChain BaseTool wrapper.

Wraps a ConstrAI ``Orchestrator`` as a LangChain ``BaseTool`` so that
ConstrAI-enforced agents can participate in LangChain chains and agent loops.

    from constrai import Orchestrator, TaskDefinition
...
LangGraph works out of the box; ``ConstrAISafeTool`` is a standard
...
# fail when langchain is not installed (constrai itself does not depend on it).
try:
...
def _require_langchain() -> None:
    if not _LANGCHAIN_AVAILABLE:
        raise ImportError(
            "LangChain is required for ConstrAISafeTool. "
            "Install it with: pip install 'constrai[langchain]'"
        )


class _ConstrAIToolInput(BaseModel if _LANGCHAIN_AVAILABLE else object):  # type: ignore[misc]
    """Input schema for ConstrAISafeTool."""
...
class ConstrAISafeTool(BaseTool):  # type: ignore[misc]
    """
    LangChain BaseTool that runs a ConstrAI Orchestrator with full safety enforcement.

    Args:
    orchestrator:
        A ConstrAI ``Orchestrator`` instance.  The orchestrator must be
    ...
    args_schema: Type[_ConstrAIToolInput] = (
        _ConstrAIToolInput if _LANGCHAIN_AVAILABLE else None  # type: ignore[assignment]
    )
    ...
    def _run(
        ...
        """
        Execute the ConstrAI Orchestrator and return a summary string.
    ...
    async def _arun(
        ...
        EXPERIMENTAL: ConstrAI's Orchestrator is synchronous. This wraps
    ...
    def __repr__(self) -> str:
        return (
            f"ConstrAISafeTool(name={self.name!r}, "
