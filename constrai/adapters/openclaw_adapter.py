"""
constrai.adapters.openclaw_adapter — OpenClaw personal AI assistant adapters.
...
**1. Direct adapters** — wrap ``openclaw agent`` for use with the ConstrAI
    Orchestrator.  No extra Python package required; only the OpenClaw CLI.

    Sync::

        from constrai.adapters import OpenClawAdapter
...
class OpenClawAdapter:
    """
    ConstrAI LLM adapter for OpenClaw (sync, subprocess-based).
...
class AsyncOpenClawAdapter:
    ...
    :class:`~constrai.AsyncSafetyKernel` or any coroutine-based orchestration.
