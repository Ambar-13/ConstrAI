"""
constrai.reasoning — Structured reasoning engine for autonomous agents.
...
reasons within these constraints; the safety kernel verifies the result.
...
# Belief state — Bayesian uncertainty tracking
...
class LLMAdapter(Protocol):
    ...
    ~~~~~~~~~~~~~~~~~~~~~~~~
    ConstrAI evaluates safety on *complete* action proposals (JSON objects).
    Evaluating a partial token stream is not meaningful — you cannot run T1–T7
    checks on half a JSON object.  Therefore:

      • ``complete()`` is the primary method: it returns the full response and
        is the only method that ConstrAI's safety kernel consumes.
      • ``stream_tokens`` is a *callback* passed to ``complete()``.  It lets
      ...
      • ``acomplete()`` is an optional async variant.  It is NOT used by the
        ...
        *,
        ...
        Args:
            ...
            stream_tokens: Optional callback invoked with each token fragment
                as it arrives. Used for streaming UX only — the safety kernel
    ...
    def acomplete(
        ...
        *,
        ...
        orchestrator never calls this method — it is provided for callers that
...
class MockLLMAdapter:
    ...
    async def acomplete(
        ...
        *,
        ...
        Delegates to ``complete()`` synchronously — no I/O occurs, so no
