"""
constrai.inverse_algebra — Exact rollback via inverse effect algebra (T7).

Computes and applies inverse effects automatically. The core theorem is
T7: undo(execute(s, a)) == s exactly. The proof is straightforward: State
is immutable (formal.py), so the snapshot stored in state_before is
guaranteed to be unmodified. Rollback returns it directly rather than
re-applying computed inverses — the algebra is used to validate and
document the inverse, not to reconstruct the state.
...
from .formal import _SENTINEL_DELETE, ActionSpec, Effect, State
...
# ActionSpec extension with built-in inverse support
