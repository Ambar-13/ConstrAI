"""
constrai.orchestrator — Main execution loop for autonomous agents.
...
class AsyncOrchestrator:
    ...
    Usage::
        ...
        from constrai import AsyncOrchestrator, TaskDefinition
    ...
    async def _ask_llm(
        ...
        try:
            ...
            self._llm_prompt_tokens += estimate_tokens(
                self.task.system_prompt or ConstrAI_SYSTEM_PROMPT
            )
            saved = max(0, estimate_tokens(full_prompt) - estimate_tokens(prompt))
            if saved:
                self._errors.append(f"prompt_savings_tokens={saved}")
