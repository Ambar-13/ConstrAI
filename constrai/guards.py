            ...
            if isinstance(l, (int, float)) and isinstance(r, (int, float)):
                ...
                try:
                    ...
                    raise GuardParseError(
                        f"Non-numeric comparison: {l!r} {self.op} {r!r}"
                    )
...
def kernel_precondition_from_actions(*, all_actions: Iterable[Any]):
    ...
    Returns a function: (state, action) -> (ok, msg)

    Note: This is *pure* and only reads builtin types.
    """
    ...
    for a in all_actions:
        ...
        compiled[getattr(a, "id")] = compile_guard(pre_s)

    def _check(state: Any, action: Any) -> Tuple[bool, str]:
        ast = compiled.get(getattr(action, "id"))
        if ast is None:
        ...
        if isinstance(state, dict):
            ...
            env = state.to_dict()  # type: ignore[assignment]
        else:
            env = getattr(state, "data", None) or getattr(state, "_data", None)
            if env is None:
                env = dict(getattr(state, "__dict__", {}))

        try:
            ok = bool(ast.eval(env))
        except Exception as e:
            return False, f"Invalid precondition for {getattr(action, 'id')}: {e}"
        if ok:
            return True, "guard ok"
        return False, f"Precondition failed for {getattr(action, 'id')}: {getattr(action, 'preconditions_text', '')}"
