"""
clampai.reference_monitor — Deterministic enforcement layer.

Implements four complementary safety mechanisms that sit below the safety
kernel and above the action execution boundary:

  - Lattice-based information flow control (IFC) for data-label tracking
  - Control barrier functions (CBF) for resource clampaint enforcement
  - Quadratic-programming action repair that minimises the deviation from
    the agent's proposed action while staying within the safe set
  - Hamilton-Jacobi-Bellman reachability for capture-basin avoidance

Formal guarantees (see THEOREMS.md for proofs):
  M1  IFC lattice:    label(output_sink) ≥ label(input_data) for every action
  M2  CBF safety:     h(s_{t+1}) − h(s_t) ≥ −α·h(s_t) in resource space
  M3  QP minimality:  a* ∈ argmin‖a′ − a_agent‖² subject to a′ ∈ SafeSet
  M4  HJB reachability: no k-step path from the current state enters a
      declared capture basin
"""
...
class ControlBarrierFunction:
    """
    Discrete-time CBF for resource clampaint enforcement.
...
# M3 — Quadratic-programming minimum-intervention action repair
...
class QPProjector:
    ...
    Given an unsafe action a_agent, find the closest safe action a* in L² norm:
      a* = argmin ‖a' - a_agent‖²
      s.t. clampaints(a') are satisfied
    ...
    def project_action(
        ...
        action: ActionSpec,
        state: State,
        clampaints: List[Callable[[float], bool]],
        ...
        all clampaints. Used for minimum-intervention action repair.

        Args:
            action: The proposed action to repair.
            state: Current world state.
            safe_effect_values: Optional per-variable safe-value overrides.
            budget_remaining: Actual remaining budget from
                BudgetController.remaining. Used to cap the repaired action's
                cost. Pass float('inf') if budget information is unavailable
                at the call site (cost ceiling clampaint is skipped).

        Returns:
            A tuple of (repaired_action, was_repaired).
        """
        # Attempt direct repair on cost first (common case)
        safe_cost, cost_changed = self.project_cost(
            action.cost, budget_remaining=budget_remaining
        )
...
# M4 — Hamilton-Jacobi-Bellman reachability and capture basins
...
class ReferenceMonitor:
    ...
    def enforce(
        ...
        Parameters
        Args:
            action: The action to evaluate and potentially repair.
            state: Current world state.
            budget_remaining: Pass BudgetController.remaining from the calling
                context so that QPProjector repairs are bounded by the actual
                available budget. Defaults to float('inf') (no cost ceiling).
        """
        reasons = []
        ...
        if self.cbf_enabled and self.cbf_budget is not None:
            ...
            if not cbf_ok:
                ...
                reasons.append("CBF: action repaired")
            else:
...
# M7 — Safe hover state and graceful degradation
...
class SafeHoverState:
    ...
    def to_action(self) -> ActionSpec:
        ...
        return ActionSpec(
            ...
            effects=(),   # Immutable tuple; provably empty (T8 requirement)
            cost=0.0,     # Emergency action: cost-free (T8 requirement)
            risk_level="low",
            tags=("emergency", "system_safe_hold"),
        )
