...
from constrai import (
    ...
    SecurityLevel, DataLabel, ReferenceMonitor, CaptureBasin, ContractSpecification, OperadicComposition,
    TaskDefinition, Orchestrator, SafeHoverState,
)
...
def example_financial_advisor():
    ...
    print(f"    Assume: >=400 remaining (100 for savings + 200 for checking)")
    print(f"    Guarantee: savings >= 100 AND checking >= 200")
    ...
    print(f"    Reference Monitor checks: IFC✓ CBF✓ HJB✓")
    ...
    if result.goal_achieved:
        ...
        print(f"\n  ⚠️  Goal not achieved (but system remained safe)")
...
def example_ifc_violation_detection():
    ...
    print(f"\nFlow assertions:")
    print(f"  Public ≤ Internal?     {public_label <= internal_label}  ✓ (allowed)")
    print(f"  Public ≤ PII?          {public_label <= pii_label}  ✓ (allowed)")
    print(f"  Internal ≤ PII?        {internal_label <= pii_label}  ✓ (allowed)")
    print(f"  PII ≤ Internal?        {pii_label <= internal_label}  ✗ (NOT allowed)")
    print(f"  PII ≤ Public?          {pii_label <= public_label}  ✗ (NOT allowed)")
...
def example_cbf_gradual_restriction():
    ...
    print(f"  Safety condition: Δh ≥ -α·h")
    ...
    for name, state_before, state_after in scenarios:
        ...
        print(f"  {name:<15} {h_now:<10.2%} {h_next:<10.2%} {delta:<10.4f} {threshold:<12.4f} {'✓' if passes else '✗':<6}")
